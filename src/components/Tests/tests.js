...
const Tests = () => {
  ...
  const [successCss, setSuccessCss] = useState(0);
  const [totalCss, setTotalCss] = useState(0);
  ...
  const handleanswer = (e) => {
    ...
    if (e.target.value === list[numberOfQuestion].correctAnswer) {
      setSuccessCss((pr) => pr + 1);
      console.log("bU");
    }
    dispatch(addStatistic({ css: { success: successCss, total: totalCss } }));
